...
description: Use when the user wants Claude Code to coordinate Chrome ChatGPT, ChatGPT Apps or connectors, local MCP, Cloudflare or other tunnels, local project access, service start/stop/status switches, or task routing between ChatGPT Pro/high-intelligence reasoning and Claude Code local execution.
---
...
- **Claude Code** is the local executor: inspect files, edit safely, run tests, manage processes, and verify artifacts.
...
- If the account has no Cloudflare DNS zone, prefer a stable Workers proxy on `workers.dev` plus a Quick Tunnel upstream. The ChatGPT app URL stays stable while Claude Code refreshes the Worker KV target after each start.
...
|---|---|---|
| Local edits, tests, git, process control, secrets, irreversible actions | Claude Code | Stronger local tool control and verification |
...
| Cross-agent critique or plan review | ChatGPT drafts, Claude Code verifies | Avoid blind trust in either side |
| Long execution with many shell/file operations | Claude Code, optionally with ChatGPT checkpoints | Saves ChatGPT tokens and keeps artifacts local |
| User-visible ChatGPT UI configuration | Claude Code via Chrome plugin | Needs logged-in browser state |
...
1. **Frame**: Claude Code summarizes the task, local constraints, and allowed root.
...
4. **Execute**: Claude Code performs local edits/tests/commands.
5. **Verify**: Claude Code checks files, logs, tests, and browser-visible outcomes.
...
- Keep repetitive local work in Claude Code and report compact checkpoints back to ChatGPT.
...
| Letting ChatGPT directly edit without review | Require Claude Code verification before final claims |
