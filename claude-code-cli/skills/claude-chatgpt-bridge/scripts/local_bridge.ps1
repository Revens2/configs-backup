...
function Ensure-Cloudflared {
  ...
  try {
    ...
    if (Get-Command curl.exe -ErrorAction SilentlyContinue) {
      & curl.exe -sSL "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-windows-amd64.exe" -o $downloadPath
    } else {
      Invoke-WebRequest -Uri "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-windows-amd64.exe" -OutFile $downloadPath -UseBasicParsing
    }
...
function Start-CloudflareTunnel {
  ...
  $tunnelArguments = 'tunnel --url "http://127.0.0.1:{0}" --no-autoupdate --no-prechecks --protocol http2 --logfile "{1}" --loglevel info' -f $Port, $logPath
  ...
  "Start" {
    ...
    try {
      if ($Tunnel -eq "cloudflare") {
        ...
        $publicBaseUrl = $PublicBaseUrl.TrimEnd("/")
        $allowedHosts = @(([uri]$publicBaseUrl).Host)
        $existingSsh = @(Get-CommandLineProcess "*ssh*-R*$Port`:127.0.0.1:$Port*")
        if ($existingSsh.Count -eq 0) {
          try {
            $sshProcess = Start-Process -FilePath "ssh" `
              -ArgumentList @("-N", "-R", "$Port`:127.0.0.1:$Port", "-o", "ServerAliveInterval=30", "-o", "ServerAliveCountMax=3", "-o", "ExitOnForwardFailure=yes", "vps-etude-nb") `
            ...
            Write-Warning "Could not automatically launch SSH reverse tunnel: $($_.Exception.Message)"
          }
        }
      } else {
