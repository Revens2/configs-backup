...
function Test-RuntimeMatchesProfile($runtime, $profile) {
  ...
  if ($profile.tunnel -in @("cloudflare-worker", "external") -and $profile.publicBaseUrl -and (([string]$runtime.publicBaseUrl).TrimEnd("/") -ne ([string]$profile.publicBaseUrl).TrimEnd("/"))) {
    return $false
  }
  return $true
}
...
function Acquire-ControllerMutex {
  $mutex = $null
  try {
    $mutex = [System.Threading.Mutex]::new($false, "Global\ClaudeChatGPTBridge.Controller")
  } catch [System.UnauthorizedAccessException] {
    $mutex = [System.Threading.Mutex]::new($false, "Local\ClaudeChatGPTBridge.Controller")
  }
  $acquired = $false
  try {
