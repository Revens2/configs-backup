...
$stateDir = Join-Path $env:TEMP ("claude-chatgpt-bridge-test-" + [Guid]::NewGuid().ToString("n"))
$taskName = "ClaudeChatGPTBridge-Test-" + [Guid]::NewGuid().ToString("n")
...
"@
  ...
  $testClaudeHome = Join-Path $stateDir "claude-home"
  $null = Invoke-ChildPowerShell -Arguments @(
    "-File", $installScript,
    "-ClaudeHome", $testClaudeHome
  )
  $installedSkill = Join-Path $testClaudeHome "skills\claude-chatgpt-bridge"
  if (-not (Test-Path -LiteralPath (Join-Path $installedSkill "SKILL.md"))) {
    throw "Installer did not copy the skill into the test Claude home."
  }
  ...
  $backupRoot = Join-Path $testClaudeHome "skills"
  $backups = @(Get-ChildItem -LiteralPath $backupRoot -Directory | Where-Object { $_.Name -like "claude-chatgpt-bridge.backup-*" })
  ...
  if (Test-Path -LiteralPath $stateDir) {
    ...
    if (-not $resolvedState.StartsWith($resolvedTemp, [System.StringComparison]::OrdinalIgnoreCase) -or
        (Split-Path -Leaf $resolvedState) -notlike "claude-chatgpt-bridge-test-*") {
