...
2. **NotebookLM d'abord** :
   ...
   - En cas de besoin de connexion / ré-authentification NotebookLM : exécuter la commande via Brave Browser (`notebooklm login --browser-cookies brave` ou ouvrir la fenêtre GUI dans Brave avec `Start-Process "C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe"`).
   - En cas d'indisponibilité de NotebookLM : ne pas boucler, passer directement en mode WebSearch/WebFetch et le signaler dans le rapport.
