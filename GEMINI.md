...
5. **Recitation** — le bloc todo est réémis en fin de chaque message tant que des tâches restent à exécuter (§ Niveau 2) ; arrêt dès que le plan et la to-do list sont complets.
...
exécution. Réémettre le bloc todo **à la toute fin de chaque message** tant qu'il reste
des tâches en cours ou à faire, pour le maintenir dans la zone d'attention récente. Dès que
le plan et la to-do list sont intégralement complétés et vérifiés, **cesser la réémission**
dans la conversation (inutile de réécrire une checklist close). Aucune tâche marquée
`completed` sans vérification effective (test, linter, typecheck). Conserver les erreurs et
stack traces passées dans `progress.md` — les supprimer fait rejouer les mêmes échecs.
